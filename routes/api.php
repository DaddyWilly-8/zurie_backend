<?php

// Each module owns and maintains its own routes.php — this file just wires
// them in. New modules get a single line added here as they're built.
require __DIR__.'/../app/Modules/Auth/routes.php';
require __DIR__.'/../app/Modules/Media/routes.php';
require __DIR__.'/../app/Modules/Product/routes.php';
require __DIR__.'/../app/Modules/Inventory/routes.php';
require __DIR__.'/../app/Modules/Customer/routes.php';
require __DIR__.'/../app/Modules/Order/routes.php';
require __DIR__.'/../app/Modules/Dashboard/routes.php';
require __DIR__.'/../app/Modules/Activity/routes.php';
require __DIR__.'/../app/Modules/Settings/routes.php';
require __DIR__.'/../app/Modules/Finance/routes.php';
require __DIR__.'/../app/Modules/Outlet/routes.php';
require __DIR__.'/../app/Modules/PriceList/routes.php';
require __DIR__.'/../app/Modules/Account/routes.php';
require __DIR__.'/../app/Modules/Supplier/routes.php';
require __DIR__.'/../app/Modules/Purchase/routes.php';
require __DIR__.'/../app/Modules/Pos/routes.php';
require __DIR__.'/../app/Modules/Report/routes.php';
require __DIR__.'/../app/Modules/Expense/routes.php';
require __DIR__.'/../app/Modules/Target/routes.php';
require __DIR__.'/../app/Modules/Faq/routes.php';
require __DIR__.'/../app/Modules/Enquiry/routes.php';
require __DIR__.'/../app/Modules/CashierSession/routes.php';
require __DIR__.'/../app/Modules/Wishlist/routes.php';
require __DIR__.'/../app/Modules/Review/routes.php';
require __DIR__.'/../app/Modules/Notification/routes.php';
require __DIR__.'/../app/Modules/Coupon/routes.php';
require __DIR__.'/../app/Modules/MeasurementUnit/routes.php';
require __DIR__.'/../app/Modules/Currency/routes.php';
require __DIR__.'/../app/Modules/Procurement/routes.php';
require __DIR__.'/../app/Modules/Vat/routes.php';
require __DIR__.'/../app/Modules/ProformaInvoice/routes.php';
require __DIR__.'/../app/Modules/Transaction/routes.php';
require __DIR__.'/../app/Modules/Delivery/routes.php';
require __DIR__.'/../app/Modules/InventoryTransfer/routes.php';
require __DIR__.'/../app/Modules/Stakeholder/routes.php';
