...
| Rate limits | `AppServiceProvider` + `throttle:login` etc. | General API: 300/min per signed-in staff user, 180/min per signed-in customer, 120/min per IP for guests (60/min throttled staff clicking through admin screens). Login 5/min per email+IP; register/forgot/reset, contact and newsletter (`public-form`, 10/min per IP) are throttled too. |
...
| Test coverage | 123 backend tests: ledger posting, checkout, GRN receive/un-receive, reconcile, idempotency, payments/receipts, journal vouchers/fund transfers, inventory transfers (all 3 types), delivery dispatch, RBAC escalation guards + a real HTTP 401/403/200 boundary, VAT (inclusive and on-top), notifications, newsletter, contact form, reviews, rate limits and the audit-log assertions below. The storefront and admin were also clicked through end to end in a real browser (Playwright) against a fresh live-like database: admin setup (settings, category, unit, product + image, supplier, PO + GRN, coupon, FAQ, target, expense), customer journey (register, wishlist, review, coupon checkout, newsletter, contact, account, forgot password), admin operations (notification bell, order status, review moderation, enquiries, cashier session + POS sale, supplier payment, reports) and the customer seeing the result | No automated frontend component/e2e suite in CI yet — the browser run is manual |
| Audit trail | Covers auth, every Transaction subtype (Payment/Receipt/JournalVoucher/FundTransfer) create+delete, Finance's LedgerGroup/Ledger/CostCenter CRUD, `finance:reconcile --fix` corrections, the manual `activity()` calls in Order/GRN/PurchaseOrder/Purchase/Delivery/InventoryTransfer/Expense/RoleService/UserService, and model-level `LogsActivity` (explicit `logOnly`, changes stored in `attribute_changes`) on Product, Category, CashierSession, ProductReview, Supplier, PriceList, PriceListItem, SalesOutlet, MeasurementUnit, Currency, CurrencyExchangeRate, Target and Media — verified in `tests/Feature/Finance/AuditTrailTest.php` and `AuditTrailCoverageTest.php` | Coupon and Stakeholder/Customer edits still aren't model-logged |
| Customer/staff split | **Done** — see §9a for the full design. `customer_accounts` table + `customer` Auth guard, fully independent of staff `users`/`web`. Backend: `CustomerAuthController`, `CustomerAccountService`, polymorphic notifications, dual password brokers, Google login moved. Account/Wishlist/Review(store)/Notification re-pointed to the customer guard; admin-only Review moderation untouched. Frontend: separate `customerAuthService`/`authService`, separate providers, new `/admin/reset-password` page. Verified: 5 reliable automated tests (`CustomerStaffSplitTest`) covering register→login→self-service, customer-can't-reach-admin (401, not just 403), staff-can't-reach-customer-routes, phone-collision rejection, and per-customer data isolation (wishlist/notifications) — plus a live curl transcript proving both guards work simultaneously in one browser and that logging the customer out leaves the staff session untouched (the exact bug `CustomerAuthService::logout()`'s `regenerate()`-not-`invalidate()` fix targets). Full 83-test suite and `finance:reconcile` still clean after this change | Nothing structural — this was the last item on the original pending list. Staff notifications are now live (`NotificationService::notifyStaff()` → every user holding the relevant permission, sent after commit: new online order, out of stock, new enquiry, review awaiting approval; customers are told when their order status changes), with `GET/PATCH/POST /admin/notifications…` feeding the admin bell. Follow-up if it comes up in practice: reconsidering whether `oauth_identities`/pre-split `stakeholders.user_id` should eventually be cleaned up once confirmed nothing legacy depends on them |
